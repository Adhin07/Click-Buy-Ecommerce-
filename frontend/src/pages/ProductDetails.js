     ...
     <div className='min-h-[200px] flex flex-col lg:flex-row gap-4'>
      ...
      <div className='h-96 flex flex-col lg:flex-row-reverse gap-4 '>
        <div className='h-[300px] w-[300px] lg:h-96 lg:w-96 bg-slate-200  relative p-2'>
          ...
          <div className='h-full'>
            {
              loading ?  (
                <div className='flex gap-2 lg:flex-col overflow-scroll scrollbar-none animate-pulse'>

               {
                 
                productImageListLoading?.map((el,index)=>{
                  return(
                    <div className='h-20 w-20 bg-slate-200 rounded' key={"productImageListLoading"+index}>
                  </div>
                  )
                })
                }
                ...
                { 
                  data?.productImage?.map((imgURL,index)=>{
                   return(
                     ...
                     <img src={imgURL} alt='' className='w-full h-full object-scale-down mix-blend-multiply cursor-pointer' onClick={()=>handleMouseEnterProduct(imgURL)}/>
                   </div>
                   )
                 })
                 }
...

      {
        loading ?(
          ...
          <p className='bg-slate-200 animate-pulse h-4 lg:h-8 rounded-full inline-block '></p>
          ...
          <div className='flex items-center gap-3 my-2 w-full'>
            ...
            <button className='bg-slate-200 h-6  lg:h-8 rounded animate-pulse w-full'>  </button>
              ...
              <div className='w-full'>
                <p className='text-slate-600 font-medium my-1 lg:h-8 bg-slate-200 h-6 rounded animate-pulse w-full'></p>
          ...
          <div className='flex items-center gap-3 my-2'>
            ...
            <button className='border-2 border-red-600 rounded px-3 py-1 min-w[120px] font-medium text-white bg-red-600 hover:bg-white hover:text-red-700 'onClick={(e)=>handleAddToCart(e,data._id)}>Add to cart</button>
          </div>
